...
// 在Salmon中，和大多数编程语言一样，该语法的规则非常简单，可以将其归为正则语言。
// 如果你愿意，你可以非常精确地使用正则表达式来识别Salmon的所有不同词组，而且还有一堆有趣的理论来支撑着为什么会这样以及它的意义。
public class Scanner {
    ...
    // Salmon支持多行字符串。这有利有弊，但禁止换行比允许换行更复杂一些，所以我把它们保留了下来。
    // 这意味着当我们在字符串内遇到新行时，我们也需要更新line值。
    // 当我们创建标记时，我们也会产生实际的字符串值，该值稍后将被解释器使用。
    // 这里，值的转换只需要调用substring()剥离前后的引号。如果Salmon支持转义序列，比如\n，我们会在这里取消转义。
